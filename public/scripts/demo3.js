...
const demo = document.querySelector('.navigation__button');
...
new Nearby(demo, {
    onProgress: (distance) => {
        const point = lineEq(7, 0, distanceThreshold.max, distanceThreshold.min, distance);
        const visible = Math.max(0,Math.floor(point));
        
        if(visible < 7){
            console.log("zone");
             $('.navigation__menu-horizontal').css('opacity','1');
            $('.navigation__menu-horizontal').addClass("navigation__menu-horizontal--animated");
           
            $('.navigation__menu-horizontal').fadeIn();
        }
        else{
            
            $('.navigation__menu-horizontal').fadeOut();
        }
